...
class UnusualWhalesAnalyzer:
    ...
    def analyze_dark_pool(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        vwap = (data['price'] * data['volume']).sum() / total_volume if total_volume > 0 else 0
        ...
        return {
            'net_flow': block_volume / total_volume if total_volume > 0 else 0,
            'block_trade_ratio': len(data[data['volume'] >= self.min_block_size]) / len(data) if len(data) > 0 else 0,
    ...
    def analyze_option_flow(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        return {
            ...
            'large_order_ratio': large_orders / total_orders if total_orders > 0 else 0,
            'bullish_flow_score': (call_premium - put_premium) / (call_premium + put_premium) if (call_premium + put_premium) > 0 else 0
        }
    ...
    def generate_signal(self, analysis: UnusualWhalesAnalysis) -> Tuple[str, float]:
        # Combine all metrics into a single signal
        bullish_factors = 0
        total_factors = 0
        
        # Dark pool analysis
        if analysis.dark_pool_metrics['net_flow'] > 0.6:
            bullish_factors += 1
        elif analysis.dark_pool_metrics['net_flow'] < 0.4:
            bullish_factors -= 1
        total_factors += 1
        
        # Options flow
        if analysis.option_flow_metrics['bullish_flow_score'] > 0.2:
            bullish_factors += 1
        elif analysis.option_flow_metrics['bullish_flow_score'] < -0.2:
            bullish_factors -= 1
        total_factors += 1
        ...
        # Greek exposure
        if analysis.greek_exposure['net_delta'] > 0:
            bullish_factors += 1
        elif analysis.greek_exposure['net_delta'] < 0:
            bullish_factors -= 1
        total_factors += 1
        
        signal_strength = abs(bullish_factors) / total_factors
